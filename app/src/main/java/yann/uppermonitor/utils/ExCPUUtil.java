...
public class ExCPUUtil {
    private static final String TAG = ExCPUUtil.class.getSimpleName();
    ...
    public static String printCpuInfo() {
        String info = ExFileUtil.getInstance().readFile(CPU_INFO_PATH);
        return info;
    }
    ...
    public static String getCpuName() {
        if (!ExCommonUtil.isEmpty(CPU_NAME)) {
            return CPU_NAME;
        }
        try {
    ...
    public static long getCurrentFreqency() {
        try {
            return Long.parseLong(ExFileUtil.getInstance().readFile(CPU_FREQ_CUR_PATH).trim());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 0;
